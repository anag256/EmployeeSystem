...
function FormComponent({action,handleSubmit,firstName,setFirstName,lastName,setLastName,email,setEmail, dob,setDOB,gender, setgender,radioCHangehandler,empID,setEmpID,handleRead,errors,setErrors}) {
    ...
    return (<>
      {!(action==='create') && <><form onSubmit={handleRead}>
       ...
       <form className='addform' onSubmit={(e)=>handleSubmit(e,action)}>
        <label>First Name</label>  <input type="text" maxlength='8' value={firstName} onChange={(e)=>setFirstName(e.target.value)} disabled={(action==='read'|| action==='delete')?true:false}/>
        ...
        <label>Email</label>   <input type="email"  value={email} onChange={(e)=>setEmail(e.target.value)} disabled={(action==='read'|| action==='delete')?true:false}></input>
